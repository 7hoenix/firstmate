...
That cadence is per pause: the fleet default `FM_PAUSE_RESURFACE_SECS`, or a longer window a captain-gated lane declares inline with a `paused [recheck=<dur>]: <why>` token (a merge or release gate that legitimately idles for hours and should not be rechecked hourly), clamped to `[FM_PAUSE_RECHECK_MIN_SECS, FM_PAUSE_RECHECK_MAX_SECS]` so no token can nag faster than the floor or push a recheck past the day ceiling; `bin/fm-classify-lib.sh`'s `pause_recheck_secs` owns the token grammar for both the watcher and the away-mode daemon.
