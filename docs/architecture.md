...
That recheck is throttled to at most one surface per `FM_PAUSE_RESURFACE_SECS`, tracked in a durable marker that survives watcher restarts and is not cleared by a plain stale surface: only an authoritative `working` verdict cancels a still-declared pause, so a current-state read that flickers to `none` (a bounded probe that timed out, a mis-attributed run) stays a throttled recheck instead of re-surfacing every cycle (the 2026-07-13 expired-pause wake loop).
