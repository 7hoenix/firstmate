...
That cadence starts from a per-pause base: the fleet default `FM_PAUSE_RESURFACE_SECS`, or a longer window a captain-gated lane declares inline with a `paused [recheck=<dur>]: <why>` token (a merge or release gate that legitimately idles for hours and should not be rechecked hourly), clamped to `[FM_PAUSE_RECHECK_MIN_SECS, FM_PAUSE_RECHECK_MAX_SECS]` so no token can nag faster than the floor or push a recheck past the day ceiling; `bin/fm-classify-lib.sh`'s `pause_recheck_secs` owns the token grammar for both the watcher and the away-mode daemon.
From that base the recheck interval then widens with asymmetric exponential backoff: each time an unchanged pause re-surfaces, the next interval doubles (`base * 2^streak`, capped at `FM_PAUSE_RESURFACE_MAX_SECS`, default 12h), so ten parked lanes are not each rechecked hourly for a whole day, while a forgotten pause still re-surfaces within the cap.
The backoff is asymmetric: the streak resets to the base interval only on a positive signal (a fresh crew status line, detected by a changed status-file mtime, or an authoritative `working` verdict on resume), never on an ambiguous read, and a supervisor ack defers at the current widened interval rather than snapping back to the base.
The streak is keyed by the logical pause, not the pane hash, so a churny idle pane cannot fabricate new pauses or reset the widening; `bin/fm-classify-lib.sh`'s `pause_backoff_secs` owns the backoff math for both supervisors so they never drift on next-due timing.
When the first mate is woken by a recheck and confirms the wait still holds, `bin/fm-pause-ack.sh <id>` touches a supervisor-owned `state/.paused-ack-<key>` marker that defers the next recheck a full window without a crew turn: the recheck age is anchored on the newer of that ack marker and the crew's status-file mtime, so an ack resets the window exactly as a fresh crew re-append would, without the full-context steer on a resident high-context crew that the re-append would cost, and without ever touching the crew's append-only status file (whose changed size:mtime signature would itself trip the signal scan).
That recheck is throttled to at most one surface per window, tracked in a durable marker that survives watcher restarts and is not cleared by a plain stale surface: only an authoritative `working` verdict cancels a still-declared pause, so a current-state read that flickers to `none` (a bounded probe that timed out, a mis-attributed run) stays a throttled recheck instead of re-surfacing every cycle (the 2026-07-13 expired-pause wake loop).
...
Pane-staleness hashing strips animated harness chrome - the context-meter sparkline and dot spinners, whose Block Elements and Braille glyphs never appear in real content - before hashing, so an idle pane whose only per-poll change is a redrawing footer stays deduplicated by `state/.stale-*` instead of surfacing a fresh stale wake every cycle, while any meaningful content change still changes the hash (the 2026-07-13 idle-pane wake loop).
...
Herdr's container shape is one workspace per task (a worker per space in herdr's spaces sidebar): a primary-home task is labeled `fm-<id>`, a secondmate-home task `2ndmate-<secondmate-id>-fm-<id>`, teardown reaps the task's own workspace, and recovery/list-live scopes to the current `FM_HOME` by label. This supersedes the earlier workspace-per-home shape; the reversal rationale and evidence are in `docs/herdr-backend.md`.
...
Zellij's container shape is simpler: one shared `firstmate` session, one tab per task; visible tab titles are scoped by the active home label plus a short hash of the resolved `FM_ROOT` path.
...
On the herdr backend, each task gets its own workspace labeled for the task, and a secondmate home's tasks carry a home-prefixed label (`2ndmate-<secondmate-id>-fm-<id>`), so that home's workers are clearly grouped in the spaces sidebar without sharing one workspace.
...
Cleanliness counts tracked files only: staged or unstaged changes to tracked files block a fast-forward, but untracked files alone never do, so an intentional dropped-in file (like the per-clone worktree-pool config) does not masquerade as unlanded work.
Dirty clones (tracked-file changes), non-default branches, detached HEADs with unique commits, diverged defaults, and default branches checked out in another worktree are reported as `STUCK:` with their behind count and left untouched.
When an incoming commit would overwrite a local untracked file, git's own fast-forward (or recovery checkout) refuses; that refusal is surfaced as a `STUCK:` carrying git's reason, and the local file is never deleted or clobbered.
