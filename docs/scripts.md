...
| `fm-sign-lib.sh`         | Disable commit and tag signing scoped to a finalized task worktree so autonomous commits never block on an interactive signer |
| `fm-nm-clone-config.sh`  | Harden a project's no-mistakes internal clone against unattended commit-signing and credential hangs, scoped repo-local to that clone only |
...
| `fm-pause-ack.sh`        | Defer a declared pause's next recheck a full window via a supervisor-owned ack marker, no crew turn |
