...
FM_BACKEND_HERDR_BARE_PROMPT_RE='^❯|^›'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›; a leading-anchored alternation, not a bracket expression, so a C/POSIX LC_CTYPE cannot byte-match a box-drawing glyph that shares a lead byte; faint Codex suggestion text after that prompt reads empty (docs/herdr-backend.md "Incident (2026-07-08)")
FM_BACKEND_HERDR_QUEUED_HINT_RE='^❯[^A-Za-z0-9]*Press up to edit queued messages?$|^›[^A-Za-z0-9]*Press up to edit queued messages?$'  # herdr-only: claude's "queued for after this turn" composer placeholder; when a mid-turn message is accepted while claude is busy the composer shows this hint, so its presence means "composer empty, message submitted" and the row reads empty regardless of de-emphasis styling; the separator-only [^A-Za-z0-9]* tolerates claude's NO-BREAK-SPACE glyph separator while refusing any real typed message that merely ends with the phrase (an unbounded .* would false-match such an unsubmitted message as submitted - a lost message) (docs/herdr-backend.md "Incident (2026-07-13)")
