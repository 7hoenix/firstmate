...
FM_BACKEND_HERDR_QUEUED_HINT_RE='^❯.*Press up to edit queued messages?$|^›.*Press up to edit queued messages?$'  # herdr-only: claude's "queued for after this turn" composer placeholder; when a mid-turn message is accepted while claude is busy the composer shows this hint, so its presence means "composer empty, message submitted" and the row reads empty regardless of de-emphasis styling; the `.*` tolerates claude's NO-BREAK-SPACE glyph separator (docs/herdr-backend.md "Incident (2026-07-13)")
