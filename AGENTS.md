...
config/workspace-setup.json  optional per-project worktree setup (toolchain/deps/secrets run on both fresh worktree creation and re-lease, idempotently); LOCAL, gitignored; firstmate-maintained but human-editable; absent or no entry for a project = zero setup for it; consumed by bin/fm-workspace-setup.sh at spawn and validated at session start; inherited by secondmate homes (docs/workspace-setup.md)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.setup.log     per-task workspace-setup output written by fm-spawn via fm-workspace-setup.sh; removed by teardown (docs/workspace-setup.md)
...
`config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, and `config/workspace-setup.json` are inherited into every secondmate home; `config/secondmate-harness` is not, because secondmates never spawn secondmates.
