...
An unchanged pause's recheck interval also widens automatically (exponential backoff, capped), so a long-idle parked lane is rechecked far less often than hourly; a captain-gated lane that idles for hours (a merge or release gate) can additionally declare a longer starting cadence with a `paused [recheck=<dur>]: <why>` token.
...
Ship briefs also carry a commit-style contract - short imperative conventional-commit subject, at most a few body lines of what and why, with the long story kept in the PR description - which `bin/fm-brief.sh` owns in full; scout briefs omit it because they produce a report rather than commits.
